...
describe('buildStorageKey', () => {
  it('produces the correct path format with bedId', () => {
    const key = buildStorageKey('farm-001', 'bed-a1', 'img-001', '2026-03-17T10:30:00.000Z');
    expect(key).toBe('images/farm-001/bed-a1/2026/03/17/img-001.jpg');
  });

  it('zero-pads month to 2 digits (January = 01)', () => {
    const key = buildStorageKey('f', 'b', 'i', '2026-01-05T00:00:00.000Z');
    expect(key).toMatch(/\/2026\/01\/05\//);
  });

  it('uses UTC date (not local date)', () => {
    // 2026-03-17T23:30:00.000Z — UTC date is 17, not 18
    const key = buildStorageKey('f', 'b', 'i', '2026-03-17T23:30:00.000Z');
    expect(key).toContain('/2026/03/17/');
  });

  it('always appends .jpg extension', () => {
    const key = buildStorageKey('f', 'b', 'img-xyz', '2026-06-15T12:00:00.000Z');
    expect(key).toMatch(/img-xyz\.jpg$/);
  });

  it('is deterministic (same input = same output)', () => {
    const args = ['farm-001', 'bed-001', 'img-001', '2026-03-17T00:00:00.000Z'] as const;
...
describe('uploadImage', () => {
  it('calls PutObjectCommand with correct bucket and key', async () => {
    ...
    expect(input.Key).toBe('images/farm-001/bed-a1/2026/03/17/img-001.jpg');
    expect(input.ContentType).toBe('image/jpeg');
    expect(storageKey).toBe('images/farm-001/bed-a1/2026/03/17/img-001.jpg');
  });

  it('returned storageKey matches buildStorageKey output', async () => {
    s3Mock.on(PutObjectCommand).resolves({});
    const returned = await uploadImage('f', 'b', 'i', '2026-06-01T00:00:00.000Z', new Uint8Array(), 'image/jpeg');
    expect(returned).toBe(buildStorageKey('f', 'b', 'i', '2026-06-01T00:00:00.000Z'));
  });
});
...
describe('getSignedImageUrl', () => {
  it('returns a URL string', async () => {
    const url = await getSignedImageUrl('images/farm-001/bed-a1/2026/03/17/img-001.jpg');
...
describe('deleteImage', () => {
  it('calls DeleteObjectCommand', async () => {
    ...
    await deleteImage('images/farm-001/bed-a1/2026/03/17/img-001.jpg');
