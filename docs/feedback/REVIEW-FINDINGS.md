# Review Findings — Session 9 (2026-04-24) — Wave E step 1 promote-legacy migration (#279)

Summary: MUST-FIX: 0 · SHOULD-FIX: 2 · SUGGESTION: 3

Scope: uncommitted Wave E step 1. One-time DDB migration that promotes beds with inline `crop_type` + no `completed_at` to persisted `BedCrop` rows stamped `created_from_legacy: true`. 3 modified files (contract add-on: type/schema/mapper), 3 new files (pure module, CLI wrapper, 15-test vitest).

| ID | Severity | Location | Finding | Recommendation |
|---|---|---|---|---|
| R-E1-001 | SHOULD-FIX | `src/api/src/services/repositories/bed-crops.ts:55-69` (`createBedCrop`) invoked from `services/migrations/wave-e-promote-legacy.ts:83` | **Race window on concurrent migration runs.** `createBedCrop` has NO `ConditionExpression`. If an operator launches the CLI twice (or a retry runs while the first is mid-scan), both processes can call `listBedCropsByBed(bedA) → []`, both pass `shouldPromoteBed → needed: true`, both PUT a fresh `created_from_legacy: true` BedCrop with a different uuid SK. The PK/SK are distinct (different uuids) so no DDB-level collision rejects the second write → bed ends up with TWO legacy-promoted crops. On a bed with 4 pre-existing active crops this also silently breaches the 5-cap invariant (S5-4) since the migration bypasses the route-layer cap check. Low probability with a single operator + single-region, but "idempotent migration" is a load-bearing claim in DESIGN-279 §6 step 1 and the current code does NOT uphold it under concurrency. | Add `ConditionExpression: 'attribute_not_exists(PK)'` to the `PutCommand` in `createBedCrop` (safe for all callers — every create path generates a fresh uuid). Additionally, inside `promoteLegacyCropForBed`, after the PUT, **re-read** `listBedCropsByBed(bed.id)` and, if >1 row carries `created_from_legacy: true`, log an error and advise cleanup. Cheap belt-and-suspenders for a one-time migration. Alternative (less invasive but narrower): add a write-scoped `ConditionExpression` via a new repo overload used only by the migration, so normal route code is unchanged. |
| R-E1-002 | SHOULD-FIX | `scripts/migrate-wave-e-promote-legacy-crops.ts:75-89` main loop | **Sequential promotion with per-bed await, no concurrency cap, no progress heartbeat.** `for (const bed of beds) { await promoteLegacyCropForBed(...) }` processes one bed at a time. For each bed we make 2 DDB calls (list + put) serialized. On a table with N beds the migration takes `N × (list_rtt + put_rtt)` real time. For ~10 farms × ~20 beds = 200 beds × ~200ms = 40s (fine); for a future tenant with 10k beds this is 40 min of wall clock under network latency with zero progress output between `Scanned N bed rows` and the final summary. The silent window is operationally risky for a one-time migration (operator can't distinguish "running" from "hung"). | Two options: (a) **progress log**: emit `console.log` every 50 beds with a running tally (cheap, 2 LOC, keeps ordering guarantees and per-bed error isolation). (b) **bounded parallelism**: batch 5 beds at a time via `Promise.all` — ~5× faster but complicates the `.catch` ordering of console messages. Option (a) is sufficient for v1.0. |
| R-E1-003 | SUGGESTION | `src/api/src/services/migrations/wave-e-promote-legacy.ts:79` | `options.idFactory` is good for tests, but the default `randomUUID()` import is hoisted to module scope. Not a bug — just calls `randomUUID` directly without a seam for the rare case an operator wants deterministic rerun ids for diffing. Acceptable for a one-time migration; flag only because the `idFactory` surface implies wider extensibility. | No action required. If you want parity with the test seam, make the default a module-level `const defaultIdFactory = () => randomUUID()` so it's mockable without passing the option. Optional. |
| R-E1-004 | SUGGESTION | `scripts/migrate-wave-e-promote-legacy-crops.ts:51` | `itemToBed(item, item['farm_id'] as string, item['id'] as string)` trusts `farm_id` and `id` are present as columns on every bed row. Verified OK via `beds.ts:118-124` (the bulk PutRequest spreads the `Bed` object which includes both). But if any hand-patched bed row pre-Wave-B lacks `farm_id`, the migration silently writes a BedCrop with `farm_id: undefined`, which `pk.farm(undefined)` renders as the string `FARM#undefined` — a malformed PK that would be unreachable via normal farm-scoped queries. | Add a guard at `scripts/migrate-wave-e-promote-legacy-crops.ts:51`: `if (!item['farm_id'] || !item['id']) { console.error('[wave-e-promote] SKIP malformed bed row:', item.PK, item.SK); continue; }`. 3 LOC, bounds blast radius. |
| R-E1-005 | SUGGESTION | `src/api/src/__tests__/services/migrations/wave-e-promote-legacy.test.ts` | Coverage is strong (15 tests hit the 4 skip reasons × 2 layers). Three minor gaps: (a) no test for `completed_at: ''` (empty string is truthy? no — it's falsy, but the current `if (bed.completed_at)` already short-circuits correctly; confirm with a dedicated case). (b) no test for `idFactory` absence in live mode (currently only dry-run-with-factory and live-with-factory are tested; the `randomUUID()` default branch is uncovered). (c) no test pinning that `createBedCrop` receives `created_by: 'system'` specifically (the current `.toHaveBeenCalledWith(expect.objectContaining({ created_from_legacy: true }))` doesn't lock the sentinel). | Optional test additions (~12 LOC total). None block Wave E ship; all three would harden against accidental future regressions. |

## Verification performed

- **Scan-filter safety (concern #3)**: `grep "SK:\s*sk\." src/api/src/services/repositories/*.ts` → only `sk.bed(...)` produces a `BED#` prefix. All other SK builders (`crop`, `image`, `tag`, `device`, `diary`, `notif`, `member`, `farmMember`, `joinRequest`, `meta`, `profile`, `settings`) yield distinct prefixes. `DDB_KEY_PREFIXES.CROP = 'CROP#'` (not `BED#`). `begins_with(SK, 'BED#')` is **safe** — zero false positives for any current entity.
- **farm_id presence (concern #4)**: `beds.ts:118-124` bulk write includes `...bed` spread where `bed.farm_id` is set from the caller-provided `farmId`. Column is therefore present on every row written by the app. Hand-edited rows remain a latent risk → R-E1-004.
- **Idempotency ordering (concern #1)**: `shouldPromoteBed` at lines 31-39 checks in order: `no-inline-crop` → `bed-completed` → `already-has-legacy` → `has-real-bedcrops` → `needed: true`. Confirmed via test `idempotency-marker takes precedence over real-bedcrop check` — single-process idempotency is sound. Multi-process race is R-E1-001.
- **Dry-run correctness (concern #2)**: `promoteLegacyCropForBed` at line 82 gates `createBedCrop` behind `if (!options.dryRun)`. No mutation outside the PUT. Test `does NOT write in dry-run mode but still returns a promoted result` locks this via `expect(mockCreateBedCrop).not.toHaveBeenCalled()`.
- **5-cap chain (concern #5)**: `shouldPromoteBed` returns `'has-real-bedcrops'` when `existingRealCrops.length > 0`. A 4-real-active + 1-inline-legacy bed therefore skips (4 > 0) and cannot be promoted to 5+1=6. Invariant holds pre-migration; post-migration by design the inline `crop_type` is left in place (to be removed in Wave E step 4).
- **`created_by: 'system'` safety (concern #6)**: `grep -rn "\.created_by\b" src/api src/frontend` → only DiaryEntry sites reference `created_by` in authz logic (`DiaryPage.tsx:742/780/793/830` + `diary.ts:139`). BedCrop `created_by` is stored + surfaced on API responses but never used in authz. `bed-crops.ts` routes use `assertBedWriteAccess(bed, userId)` (bed-level ownership), not crop-level. Sentinel is **safe**.
- **Scan cost (concern #7)**: Small dataset in v1.0. Non-blocking.
- **Error-recovery granularity (concern #8)**: Per-bed `.catch` returning `null` + `continue` is correct given R-E1-001 fix + migration idempotency guarantee. No action.

## Accept / Block

**Status**: ACCEPT with SHOULD-FIX on R-E1-001 before first production run. R-E1-001 is not blocking for the code commit itself, but MUST be addressed before the CLI is executed against prod DynamoDB (either by adding the conditional write, or by operational discipline of "only one operator runs this, ever"). R-E1-002 is nice-to-have for ops UX. R-E1-003/004/005 are optional polish.
...
Scope: uncommitted changes on `develop` re-keying the diary overlay (actual bars + event dots) from `bed_id` to `bed_crop_id ?? bed_id`. Four files, 81 lines added / 21 removed.

| ID | Severity | Location | Finding | Recommendation |
|---|---|---|---|---|
| R-D6-001 | SHOULD-FIX | `src/frontend/src/components/CropTimeline.tsx:74` | Comment reads `// Actual bar position (per-crop when cropId set, else bed-level)`. The phrase "bed-level" survives here after being deliberately removed from the equivalent comment in `GanttChart.tsx:133`. The task checklist called out removing stale `bed-level` language in affected files; the grep hit confirms one straggler. Not a correctness issue — functionally identical to the consumer change — but inconsistent with the sibling component and with the task's own stated cleanup goal. | Rewrite to mirror `GanttChart.tsx` phrasing, e.g. `// Actual bar position — keyed per-crop when row.cropId is set, falls back to bedId for legacy/virtual rows` (same cascade note as the map builders' JSDoc). Keeps the two components' comments in lockstep. |
| R-D6-002 | SUGGESTION | `src/frontend/src/lib/diary-utils.ts:127-146` + orphaned legacy scenario | A pre-D1 diary entry (`bed_crop_id: null`) on a bed that NOW has real BedCrops will be keyed under `bed_id` and bucket-isolated. But `DiaryPage.ganttRows` (lines 505-534) emits the virtual-legacy row (`cropId: null`) ONLY when `bedCrops.length === 0` — so once any BedCrop exists on the bed, no row renders with `cropId=null`, and the cascade `row.cropId ?? row.bedId` resolves to each crop's id, never to `bedId`. The pre-D1 entry becomes invisible on Gantt/Timeline. This matches the memory-documented "no data migration" policy (legacy entries stay on the virtual-legacy row; if the bed has a real BedCrop, there's no virtual row) and is therefore acceptable. BUT the behavior is not documented anywhere in the affected files, and a future reader will find it surprising. | Add a one-line comment near the `map.set(key, existing)` in `buildActualDatesMap` (or in the producer JSDoc) stating: "Entries with `bed_crop_id: null` on a bed that has real BedCrops will not match any row (no virtual-legacy row is emitted) — these are pre-D1 entries and are considered orphaned by design (see memory: `project_462_phase2_skipped`)." Alternatively, note it in `DiaryPage.tsx:498-534` where the row-emission policy lives. Either location is fine; the current docs do not mention it. |
| R-D6-003 | SUGGESTION | `src/frontend/src/__tests__/diary-utils.test.ts:423-450, 557-584` | The 6 new tests cover the three documented D6 scenarios (per-crop key, sibling isolation, mixed coexistence) symmetrically across both builders — good. Not covered: (a) the orphaned-legacy case called out in R-D6-002 — i.e. when a `bed_crop_id: null` entry is bucketed under `bed_id` but no row consumer queries that key because the bed has real BedCrops; (b) a `bed_crop_id` that's the empty string `""` — currently treated as truthy by `??`, so it would become the key; backend validation should prevent this but the cascade does not guard against it. Neither is blocking. | Optional: add a test documenting the orphan scenario as an integration-level assertion (or skip — it's arguably a DiaryPage concern, not a diary-utils concern). For empty-string defense, a single `expect(...).toBeUndefined()` on `map.get("")` would lock the contract; otherwise rely on backend validation + the `DiaryEntry.bed_crop_id` schema. |

## Verification performed

- **Cascade symmetry**: producer (`diary-utils.ts:132, 183`) uses `entry.bed_crop_id ?? entry.bed_id`; consumers (`GanttChart.tsx:134`, `CropTimeline.tsx:41, 76`) use `row.cropId ?? row.bedId`. Both sides fall back to the bed when the crop side is null — round-trip correct. Legacy chain: a `bed_crop_id=null` entry is stored under `bed_id=X`; a virtual-legacy row has `cropId=null, bedId=X`; `map.get(null ?? X) === map.get(X)` → hits the bucket. Verified end-to-end.
- **Collision safety**: backend generates `bed_crop_id` via `crypto.randomUUID()` at `src/api/src/routes/bed-crops.ts:92` (and `bed.id` the same way at `src/api/src/routes/beds.ts:338`). Both are UUIDv4, 122 bits of entropy each, drawn from the same global namespace — cross-bucket collision probability is cryptographically negligible (≈2⁻¹²²). Cascade is safe.
- **Stale-comment grep**: `grep -n "until Wave D\|Wave D scope\|bed-level" CropTimeline.tsx GanttChart.tsx diary-utils.ts` → one hit, `CropTimeline.tsx:74` (see R-D6-001). No other stragglers in the affected files or elsewhere under `src/frontend/src/`.
- **Test shape check**: both describe-local `makeEntry` helpers (`diary-utils.test.ts:318, 456`) default `bed_crop_id: null`. Existing tests that omit `bed_crop_id` in `overrides` get the null default → cascade resolves to `bed_id` → tests keep passing. No existing test relies on `bed_crop_id` being unset-as-in-missing vs. null (TS type is `string | null`, never `undefined`), so no regression risk. The top-of-file `makeEntry` (line 21) also now declares `bed_crop_id: null` at line 33 — consistent.
- **Component contract (`DiaryPage.tsx:498-534`)**: `ganttRows` emits `cropId: crop.id` for real BedCrops and `cropId: null` only when `bedCrops.length === 0` (legacy fallback row). Confirms the orphaned-legacy scenario (R-D6-002) is an artifact of this row-emission policy, not a D6 regression — the invariant matches prior waves.
- **Type safety**: `Map<string, …>` keys remain strings (UUIDs); the `??` cascade cannot produce `null` because the producer guards `if (!entry.bed_id) continue;` at lines 128 and 173.
- **A11y / UX**: no changes to keyboard, focus, or aria semantics. A crop row with no entries correctly gets `map.get(cropId) === undefined` → zero bar + zero dots, matching the documented empty-state.

## Accept / Block

No MUST-FIX. **D6 is acceptable to commit** as-is. R-D6-001 (stale "bed-level" comment) is a 5-second polish fix worth squashing in before the commit. R-D6-002 and R-D6-003 are documentation/coverage suggestions; both are safe to defer to a Wave-D close-out pass.
