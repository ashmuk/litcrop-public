...
## Field Evaluation Plan — April 2026

**Target:** Real-world MVP evaluation with at least one camera node integrated, deployed on an actual farm in Nagano.

**Purpose:** The system has been tested in development and via browser, but never with real hardware, real crops, and real environmental conditions. A field evaluation is the only way to validate whether the system actually works end-to-end — from camera capture to thumbnail generation to AI crop advice.

**What this tests that browser testing cannot:**
- Camera node → S3 upload → thumbnail Lambda trigger → image appears in app
- Weather data accuracy for the actual farm location
- AI chat quality with real farm context (requires LLM API key)
- Mobile UX in outdoor conditions (sunlight readability, GPS accuracy, touch with gloves)
- Data volume: how does DynamoDB perform with daily image uploads over weeks?
- Battery/connectivity: camera node reliability in rural Japan

**Prerequisites before field evaluation:**
- [ ] Phase 2 + Phase 3 complete (bugs fixed, tech debt cleared)
- [ ] Real Anthropic API key configured (SSM + runtime fetch)
- [ ] At least one camera node hardware sourced and configured
- [ ] Camera node integration tested in lab (S3 upload + trigger works)
- [ ] IoT service/guide page available (Phase 5 N2, or documentation)
- [ ] Multi-farm support (Phase 4) — or accept single-farm limitation for evaluation

**Evaluation criteria:**
- Can a farmer (non-technical user) complete the full journey: register → create farm → view crops → see weather → get AI advice?
- Does the camera node reliably upload images daily?
- Does the thumbnail pipeline work (S3 trigger → Lambda → thumbnail)?
- Is the AI chat advice useful with real farm data and weather?
- What breaks? What's confusing? What's missing?

**Timeline consideration:** April is spring planting season in Nagano — ideal timing. Cold-tolerant crops (lettuce, spinach, daikon) are being planted. The system can start monitoring from day one of the growing season.

**Opinion:** This is the right approach. Software evaluation in a browser only tests the happy path. Field evaluation exposes the real failure modes — network drops, hardware quirks, UX friction in context, and whether the AI advice is actually useful to a farmer making decisions. The MVP engineering is solid enough to support this; the question is whether the UX guides the farmer effectively. April gives us ~2 weeks to complete Phase 2-3 and prepare the hardware integration.

---

> Generated by Claude Opus 4.6 | MVP Status Report | 2026-03-21
> Updated: 2026-03-21 — added field evaluation plan for April 2026
